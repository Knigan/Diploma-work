...
def empirical_error(y_true: Iterable[float], y_pred: Iterable[float]) -> Union[float, np.ndarray]:
    """
    Computes the empirical error between the ground truth
...
def r_squared(y_true: Iterable[float], y_pred:
              Iterable[float]) -> Union[float,
              np.ndarray]:
    """
...
def mean_absolute_error(y_true: Iterable[float], y_pred:
           Iterable[float]) -> Union[float, np.ndarray]:
    """
    Computes the Mean Absolute Error (MAE) between the
    ground truth values and the predicted ones.

    :param y_true:  Iterable[float], ground truth values
    :param y_pred:  Iterable[float], predicted values
    :return:  Union[float, np.ndarray], Mean Absolute
                                        Error
    """
...
def relative_mean_absolute_error(y_true: Iterable[float],
    y_pred: Iterable[float]) -> Union[float, np.ndarray]:
    """
    Computes the relative Mean Absolute Error (rMAE)
    between the ground truth values and the predicted
    ones.

    :param y_true:  Iterable[float], ground truth values
    :param y_pred:  Iterable[float], predicted values
    :return:  Union[float, np.ndarray], relative Mean
                                        Absolute Error
    """
...
def leave_one_out_error(sample: Iterable[float],
                        basis: Any,
                        y_true: Iterable[float],
                        y_pred: Iterable[float]) -> Union[float, np.ndarray]:
    """
    Computes the leave-one-out error between the ground
    truth values and the predicted ones using an
    information matrix of the PC expansion.

    :param sample:  Iterable[float], experimental design
                                     samples
    :param basis:  Any, PC basis
    :param y_true:  Iterable[float], ground truth values
    :param y_pred:  Iterable[float], predicted values
    :return:  Union[float, np.ndarray], leave-one-out
                                        error
    """
    ...
    h_diag = np.diag(psi_mat @ np.linalg.inv(psi_mat.T @
                     psi_mat) @ psi_mat.T)

    if np.sum(np.isinf(h_diag) | np.isnan(h_diag) |
              np.isclose(h_diag, 1.)) > 0:
        raise RuntimeError("The diagonal of the regression matrix contains either Inf, NaN or ones.")

    return np.mean((y_true - y_pred) ** 2 /
           (1 - h_diag.reshape(-1, 1)) ** 2, axis=0)

def relative_leave_one_out_error(sample: Iterable[float],
    basis: Any, y_true: Iterable[float], y_pred:
    Iterable[float]) -> Union[float, np.ndarray]:
    """
    Computes the relative leave-one-out error between the
    ground truth values and the predicted ones using an
    information matrix of the PC expansion.
    ...
    :param y_true:  Iterable[float], ground truth values
    :param y_pred:  Iterable[float], predicted values
    :return:  Union[float, np.ndarray], relative leave
                                        one-out error
    """
    if isinstance(y_true, Iterable) and isinstance(y_pred, Iterable) and \
       isinstance(sample, Iterable):
    ...
    e_loo = leave_one_out_error(sample, basis,
                                y_true, y_pred)
    var = np.sum((y_true - np.mean(y_true)) ** 2, axis=0) / (len(y_true) - 1)

    # If y_true is constant, then the score isn't finite:
    # it is either NaN(perfect predictions) or –Inf
    # (imperfect predictions). To prevent such non-finite
    # numbers, y default these cases are replaced with
    # 1.0 (perfect predictions) or 0.0 (imperfect
...
def q_squared(sample: Iterable[float], basis: Any,
    y_true: Iterable[float], y_pred: Iterable[float]) -> Union[float, np.ndarray]:
    """
    ...
    return 1 - relative_leave_one_out_error(sample,
               basis, y_true, y_pred)
