...
class DampedOscillator:
    def __init__(self, **kwargs):
        ...
        if any(map(lambda x: x <= 0, [self.m, self.k,
                               self.f, self.omega])):
    ...
    def solve(self) -> tuple:
        ...
        :return: tuple, real parts of displacement and
                 velocity
        """
        ...
        sol = sp.dsolve(eq, func=y(t),
              ics={y(0): self.y_0,
              sp.diff(y(t), t).subs(t, 0): self.y_1})

        self.displacement = sp.re(sol.rhs)
        self.velocity = sp.re(sp.diff(self.displacement,
                                      t))
    ...
    def lambdify_displacement(self, c: float = 0.1) -> callable:
        """
        Creates a Python function out of the displacement
        expression.

        :param c:  float, value for the undetermined
                          damping factor
        :return:   function, for the displacement time
        ...
        t = sp.Symbol('t', real=True, positive=True)
        return sp.lambdify(t,
               self.displacement.subs(self.c, c,
               real=True), 'numpy')

    def lambdify_velocity(self, c: float = 0.1) -> callable:
        """
        Creates a Python function out of the velocity
        expression.

        :param c:  float, value for the undetermined
                          damping factor
        :return:   function, for the velocity time
                             dependence computation
        """
        ...
        t = sp.Symbol('t', real=True, positive=True)
        return sp.lambdify(t, self.velocity.subs(self.c,
                           c, real=True), 'numpy')

    def compute_displacement(self, c: float = 0.1,
                             coordinates: Iterable[float]
                             = None) -> np.ndarray:
        """
        Computes the displacement values at the
        coordinate points using a certain
        value of the damping factor.

        :param c:  float,  damping factor
        :param coordinates:  Iterable[float], time points
        :return:  np.ndarray, displacement values at the
                              coordinates
        """
    ...
    def compute_velocity(self, c: float = 0.1,
                         coordinates: Iterable[float]
                         = None) -> np.ndarray:
        """
        Computes the velocity values at the coordinate
        points using a certain value of the damping
        factor.

        :param c:  float, damping factor
        :param coordinates:  Iterable[float], time points
        :return:  np.ndarray, velocity values at the
                              coordinates
        """
...
def galerkin_ic(y_0: Union[int, float], v_0: Union[int,
                float], basis: Any) -> np.ndarray:
    """
    Computes the initial condition of the variables for
    the Galerkin method.

    :param y_0:  Union[int, float], displacement initial
                                    value
    ...
    if np.isnan([y_0, v_0]).any() or np.isinf([y_0,
                                      v_0]).any():
...
def galerkin_system(t: Union[int, float], x: np.ndarray,
    dist: Any, c_mu: Union[int, float], c_sigma:
    Union[int, float], basis: Any, osc: DampedOscillator) -> np.ndarray:
    """
    Computes the system of ordinary linear explicit first
    order differential equations

    :param t:  Union[int, float], time point
    :param x:  np.ndarray, variables of the differential
                           equation system
    :param dist:  Any, transformed distribution the PC
                       basis is built upon
    :param c_mu:  Union[int, float], mean of the
                  untransformed initial distribution
    :param c_sigma:  Union[int, float], standard
                     deviation of the untransformed
    ...
    for n in range(len(basis)):
        dvdt[n] = (osc.f * np.cos(osc.omega * t) *
                  delta[0, n] - c_mu * v[n] - osc.k *
                  y[n]) / osc.m
        summa = 0
        for i in range(len(basis)):
            summa += v[i] * chaospy.E(basis[1] * basis[i]
                     * basis[n], dist)

        dvdt[n] += -c_sigma / osc.m * summa
